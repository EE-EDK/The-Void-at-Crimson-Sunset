...
from tools.comic.anchor import distribute_anchors, apply_alignment

DEFAULT_HTML = "act1-fifth-harmonic.html"
DEFAULT_MANIFEST = "assets/comic/act1.comic.json"
# Curated beat->paragraph alignment (semantic). Authoritative when present;
# otherwise fall back to the heuristic even-distribution.
DEFAULT_ALIGNMENT = "assets/comic/act1.alignment.json"
...
def main(argv=None):
    ...
    p.add_argument("--alignment", default=DEFAULT_ALIGNMENT)
    ...
    if align_path.exists():
        mapping = json.loads(align_path.read_text(encoding="utf-8"))
        new_html, anchored = apply_alignment(html, beats, mapping)
        Path(args.html).write_text(new_html, encoding="utf-8")
        print(f"anchored {anchored}/{len(beats)} beats from curated alignment {align_path}")
    else:
